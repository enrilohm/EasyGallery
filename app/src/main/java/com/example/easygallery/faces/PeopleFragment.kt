...
import com.example.easygallery.map.ClusterImagesSheet
...
class PeopleFragment : Fragment() {
    ...
    private var rawClusters: List<FacesStore.StoredCluster> = emptyList()
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            val (clusters, count, bboxes) = withContext(Dispatchers.IO) {
                ...
                if (recluster || !FacesStore.hasClusters()) {
                    ...
                    val faces = FacesStore.getAllGoodFaceEmbeddings(minDetectionScore)
    ...
    private inner class ClustersAdapter(private val items: List<FacesStore.StoredCluster>) :
