...
class GalleryViewModel : ViewModel() {
    ...
    fun addPersonInclude(clusterId: Long, label: String, context: Context) {
        Thread {
            ...
            personIncludePaths = newIncludeIds.map { VectorStore.getPathsForCluster(it) }
                .reduceOrNull { acc, s -> acc intersect s } ?: emptySet()
            personExcludePaths = newExcludeIds.flatMap { VectorStore.getPathsForCluster(it) }.toSet()
